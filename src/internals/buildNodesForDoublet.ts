 ...
 * @returns 
 */
export function buildNodesForDoublet(board: BoardState, dicePip: DicePip): BoardStateNode {
    const unusedDices = dices(dicePip, dicePip, dicePip, dicePip)
    const [node,] = buildNodesForDoubletRec(board, unusedDices, dicePip, [], [], false);
    return node;
}
 ...
 * @returns 
 */
function buildNodesForDoubletRec(board: BoardState, unusedDices: Dice[], pip: DicePip, usedDices: Dice[], lastMoves: Move[], isAlreadyRedundant: boolean): [BoardStateNode, number] {
    ...
    const nodeBuilder: NodeBuilder = unusedDices.length === 2 ?
        leaveNodesAndParentBuilder(pip, dicesUsedUp, lastMoves, isAlreadyRedundant)
        : recursiveNodeBuilder(unusedDices, pip, dicesUsedUp, lastMoves, isAlreadyRedundant)

    // 各ポイントについて、子ノードを構築する
    const [major, marked] = applyDicePipToPoints(board, pip, nodeBuilder, unusedDices.length)

    // 子ノードで使えなかったダイスの数＝markedを、使用不可としてマークする
    const markAfter = unusedDices.length - marked
    const dices = usedDices.concat(Array.from({ length: unusedDices.length }, (_, idx) => {
...
function recursiveNodeBuilder(unusedDices: Dice[], pip: DicePip, usedDices: Dice[], lastMoves: Move[], isAlreadyRedundant: boolean): NodeBuilder {

    return (board: BoardState, move: Move) => {
        return buildNodesForDoubletRec(board,
            unusedDices.slice(1),
            pip,
            usedDices,
...
function lastMoveIsRedundant(lastMoves: Move[], move: Move): boolean {
    return ((lastMoves.length > 0) &&
        (move.from < lastMoves[lastMoves.length - 1].from))
}
...
function leaveNodesAndParentBuilder(
    ...
    return (board: BoardState, moveBeforeFinal: Move) => {
        ...
        if (isEog) {
            ...
            return buildNodeForEoG(board, usedDices.concat({ pip, used: true }), lastMovesForLeave, 1)
        }

        // 最後のpipを適用して末端ノードを生成する
        const isAlreadyRedundantForLeave = (isAlreadyRedundant || lastMoveIsRedundant(lastMoves, moveBeforeFinal))
        const nodeBuilder = leaveNodeBuilder(usedDices, pip,
            finalMove => lastMovesForLeave.concat(finalMove),
            finalMove => (isAlreadyRedundantForLeave || lastMoveIsRedundant(lastMovesForLeave, finalMove)))
