 ...
 * @returns 
 */
export function buildNodesForHeteroDice(board: BoardState, dice1: DicePip, dice2: DicePip): BoardStateNode {
    if (dice1 === dice2) {
    ...
    // 大きい目を先に使った場合の候補手
    const [majorTmp, majorMarked] = applyMajorPipFirst(board, majorPip, minorPip)

    // 小さい目を先に使う場合、上記の結果を踏まえた冗長判定がある
    const [minorTmp, minorMarked] = applyMinorPipFirst(board, majorPip, minorPip, majorTmp);
    ...
    if (majorMarked === 0) { // 大ぞろめにきい目を先に使えば、全てのダイスが使える
        major = majorTmp;
        minor = (minorMarked === 0) ? // すべてのダイスを使えるなら、小さい目を先に使っても良い
            minorTmp : () => NO_MOVE;
        usableDice = [{ pip: dice1, used: false }, { pip: dice2, used: false }];
    } else if (minorMarked === 0) { // 小さい目を先に使えば、すべてのダイスが使える
        minor = minorTmp;
        ...
        if (majorMarked === 1) {
            major = majorTmp;
            minor = () => NO_MOVE;
            usableDice = [{ pip: majorPip, used: false }, { pip: minorPip, used: true }];
        } else if (minorMarked === 1) {
            major = () => NO_MOVE;
            minor = minorTmp;
            usableDice = [{ pip: minorPip, used: false }, { pip: majorPip, used: true }];
        } else { // どの目も使えない
            major = () => NO_MOVE;
            minor = () => NO_MOVE;
            usableDice = [{ pip: majorPip, used: true }, { pip: minorPip, used: true }];
        }
    }
...
function applyMajorPipFirst(board: BoardState, majorDice: DicePip, minorDice: DicePip) {
    ...
    return applyDices(board, majorDice, minorDice, isRedundantForMajorPipFirstCase);
}

// 小さい目を先に使う場合の手のリストアップ
function applyMinorPipFirst(board: BoardState, majorDice: DicePip, minorDice: DicePip, majorTmp: (pos: number) => BoardStateNode | NoMove) {
...
function applyDices(board: BoardState, firstDice: DicePip, secondDice: DicePip,
    isRedundantFunc: (move1: Move, move2: Move) => boolean = () => false): [(pos: number) => (BoardStateNode | NoMove), number] {
...
function leaveNodeAndParentBuilder(
    ...
    return (board: BoardState, firstMove: Move) => {
        ...
        if (isEog) {
            ...
            return buildNodeForEoG(board, [usedDice, { pip, used: true }], lastMovesForLeave, 0)
        }

        // 二つ目のpipを使って末端のノードを構築する
        const nodeBuilder = leaveNodeBuilder([usedDice], pip,
            secondMove => [firstMove, secondMove],
            secondMove => isRedundantFunc(firstMove, secondMove))

        const [major, unusedDices] = applyDicePipToPoints(board, pip, nodeBuilder, 1)
