...
export function toMatchID(matchState: MatchState) {
    ...
    const bit1_4 = 15 & Math.log2(cubeState.value)
    ...
    const bit16_18 = 7 & dice1
    const bit19_21 = 7 & dice2
    ...
    const bit22_36 = 31 & matchState.matchLength
    ...
    const bit37_51 = 31 & matchState.matchScore.redScore
    const bit52_66 = 31 & matchState.matchScore.whiteScore
    ...
    toEnc.reduce(
        (prev, value, idx, arr) => {
            let { lastBit: curBit, pos } = prev
            const { lastLen, dataView } = prev
            const { bit } = value
            const { len = 1 } = value
            ...
            // bitが1バイト分に満たないうちは、追加されたbitを保持していく
            // ただし、データの最後になったら、とにかく1バイト分書く
            let curLen = idx == arr.length - 1 ? 8 : lastLen + len
            if (curLen < 8) {
                return {
                    lastBit: (curBit << len) | bit,
                    lastLen: curLen,
                    dataView,
                    pos,
                }
            ...
            if (lastLen > 0) {
                ...
                const valueToWrite =
                    (curBit << shiftForLast) | (bit >> shiftForCur)

                dataView.setUint8(pos, invert(valueToWrite))

                pos += 1
                curLen -= 8
                curBit = (2 ^ (shiftForCur - 1)) & bit
            } else {
                curBit = bit
            }

            // 新規追加のbitから1バイト分書ける分を全部書き出す
            while (curLen >= 8 && pos < buffer.byteLength) {
                ...
                pos += 1
                curLen -= 8
                curBit = (2 ^ (shiftForCur - 1)) & curBit
            }

            // 1バイトに満たない分を次に引き継ぐ(lastLen<8が保証される)
            return { lastBit: curBit, lastLen: curLen, dataView, pos }
        ...
        { lastBit: 0, lastLen: 0, dataView, pos: 0 }
    )
