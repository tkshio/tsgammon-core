...
import { eogGameRecord, GameRecord, GameRecordEoG, initGameRecord } from "./GameRecord";
 ...
 * @returns 更新後の記録
 */
export function addPlyRecord<T>(matchRecord: MatchRecord<T>, plyRecord: PlyRecordInPlay, state: T): MatchRecord<T> {
    return {
        ...matchRecord,
        score: matchRecord.curGameRecord.scoreBefore,
        curGameRecord: {
            ...matchRecord.curGameRecord,
            plyRecords: matchRecord.curGameRecord.plyRecords.concat({ plyRecord, state })
        },
    }
}
 ...
 * @returns 更新後の記録
 */
export function recordFinishedGame<T>(matchRecord: MatchRecord<T>): MatchRecord<T> {
    if (matchRecord.curGameRecord.isEoG) {
        return {
            ...matchRecord,
            gameRecords: matchRecord.gameRecords.concat(matchRecord.curGameRecord),
 ...
 * @returns 更新後の記録
 */
export function trimPlyRecords<T>(matchRecord: MatchRecord<T>, index: number): MatchRecord<T> {
    if (0 <= index && index < matchRecord.curGameRecord.plyRecords.length) {
        return {
            ...matchRecord, curGameRecord: {
                ...matchRecord.curGameRecord,
                plyRecords: matchRecord.curGameRecord.plyRecords.slice(0, index),
                isEoG: false,
            }
        }
    } else {
 ...
 * @returns 更新後の記録
 */
export function setEoGRecord<T>(matchRecord: MatchRecord<T>, eogRecord: PlyRecordEoG): MatchRecord<T> {
    if (matchRecord.curGameRecord.isEoG) {
        return matchRecord
    }
