...
export function formatMatchRecord(
    ...
    redPlayer: string = "Red", whitePlayer: string = "White"
): string {
    ...
    const curGameText = (matchRecord.gameRecords.length === 0 ? "" : "\n\n\n\n")
        + formatGameRecord(matchRecord.curGameRecord, matchRecord.gameRecords.length, redPlayer, whitePlayer)
            .join('\n')
...
function formatGameRecord(
    ...
    gameNum: number,
    redPlayer: string = "Red", whitePlayer: string = "White"): string[] {
    ...
    const turns = buildTurns(plyRecords.map(rec => rec.plyRecord))
        .map((turn, index) => formatTurn(turn, index))

    const stakes = gameRecord.isEoG ? formatStakeLine(gameRecord.eogRecord.stake) : ""
    ...
    function formatScoreDesc(scoreBefore: Score) {
        const whiteCol = (`${whitePlayer} : ` + scoreBefore.whiteScore).padEnd(numWidth + 1 + columnWidth)
    ...
    function formatTurn(turn: Turn, index: number) {
        ...
        const whiteCol = (turn.white ? formatPlyRecord(turn.white, MoveFormatDirection.RELATIVE_DEC, false, "Cannot Move") : "").padEnd(columnWidth)
        const redCol = (turn.red ? formatPlyRecord(turn.red, MoveFormatDirection.RELATIVE_DEC, false, "Cannot Move") : "")
...
type Turn = {
    white?: WhiteRecord, red?: PlyRecord
}
...
function buildTurns(recs: PlyRecordInPlay[]): Turn[] {
    const { turns, lastWhite }: TmpTurns = recs.reduce(
        (prev: TmpTurns, plyRecord: PlyRecordInPlay)
            : TmpTurns => {
            const cur = plyRecord
            if (prev.lastWhite === undefined) {
                ...
                if (cur.isRed) {
                    ...
                    return { turns: prev.turns.concat({ white: prev.lastWhite, red: cur }) }
                } else {
                    // さらにWhiteが来てしまったので、保留していた分をredなしの記録として格納、
                    // 後から来た分は保留する
                    const lastWhite = cur as WhiteRecord
                    return { turns: prev.turns.concat({ white: prev.lastWhite }), lastWhite }
                }
            }
