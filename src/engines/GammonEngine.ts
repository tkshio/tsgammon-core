...
export type GammonEngine = {
     ...
     */
    cubeAction(boardState: BoardState, cubeState: CubeState): { isDouble: boolean }
     ...
     */
    cubeResponse(boardState: BoardState, cubeState: CubeState): { isTake: boolean }
...
export function randomEngine(): GammonEngine {
    return {
        ...
        cubeAction(_boardState: BoardState, _cubeState: CubeState): { isDouble: boolean } {
            return { isDouble: false }
        },
        cubeResponse(_boardState: BoardState, _cubeState: CubeState): { isTake: boolean } {
            return { isTake: true }
        },
 ...
 */
export function simpleEvalEngine(evaluate: (board: BoardState) => number): GammonEngine {
...
export function simpleEvalEngineWithEvaluator(ev: Evaluator): GammonEngine {
    return {
        initialized(): void { ev.initialize() },
        cubeAction(board: BoardState, _cubeState: CubeState): { isDouble: boolean } {
            return { isDouble: (ev.evaluate(board) > 0.40) }
        },
        cubeResponse(board: BoardState, _cubeState: CubeState): { isTake: boolean } {
            return { isTake: (ev.evaluate(board) > -0.5) }
        },
        checkerPlay(node: BoardStateNode): BoardStateNode {
            const candidates = collectNodes(node).filter(node => !node.isRedundant)
            const bestEv = candidates
                .map(node => ({ node, e: ev.evaluate(node.board) }))
                .reduce(
                    (prev, cur) => (prev.e > cur.e) ? prev : cur,
                    { node, e: Number.NEGATIVE_INFINITY }
                )
            return bestEv.node
        },
        endOfGame(boardStateNode: BoardStateNode): void { ev.endOfGame(boardStateNode) },
    };
}
