...
import { AbsoluteMove, absoluteMovesRed, absoluteMovesWhite } from "../AbsoluteMove"
...
function runAutoMatch() {
    ...
    while (!isEoG(boardStateNode)) {
        const { boardStateNode: nextNode, lastPly } = doPlay(boardStateNode, !ply.isRed)
    ...
    const stake = (ply.isRed ? scoreAsRed : scoreAsWhite)(eogStatus.calcStake(1, conf.jacobyRule))
...
function doCheckerPlay(boardState: BoardState, roll: DiceRoll, isRed: boolean) {
    ...
    const amoves: AbsoluteMove[] =
        (isRed ? absoluteMovesRed : absoluteMovesWhite)(nextNode.lastMoves())
